"""
==============================================================================
File: n03(1)_train_model_with_peak_weight.py
...
from models.Transformer import Transformer, Transformer_encoder
...
def train_model(path, input_window, output_window, 
    ...
    peak_threshold = torch.quantile(torch_train_y, 0.95)
    criterion.peak_threshold = peak_threshold.item()
    ...
    if model1 is not None: 
        best_model1_path = f'results/{path_name}/{path_name}_hybrid_{model1.__class__.__name__}_with_{model2.__class__.__name__}_({input_window},{output_window})_peak_weight.pth'
        best_model2_path = f'results/{path_name}/{path_name}_hybrid_{model2.__class__.__name__}_with_{model1.__class__.__name__}_({input_window},{output_window})_peak_weight.pth'
    else:
        best_model1_path = None
        best_model2_path = f'results/{path_name}/{path_name}_{model2.__class__.__name__}_({input_window},{output_window})_peak_weight.pth'
...
if __name__ == "__main__":
    ...
    model2 = "LSTM" # "LSTM" or "Transformer_encoder" or "Transformer"
    num_epochs = 500
    batch_size = 512
    # criterion = nn.HuberLoss(delta=1.0, reduction="mean") # Huber Loss
    criterion = WeightedHuberLoss(delta=1.0, peak_threshold=None, peak_weight=3.0) # Huber Loss
